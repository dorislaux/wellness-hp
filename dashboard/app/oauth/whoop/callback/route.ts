import { exchangeWhoopCode, getWhoopProfile } from "../../../providers/whoop";
...
function page(title: string, message: string, status = 200): Response {
  ...
  return new Response(
    `<!doctype html><html lang="en"><meta name="viewport" content="width=device-width"><title>${safeTitle}</title><body style="font-family:system-ui;padding:2rem;max-width:36rem;margin:auto"><h1>${safeTitle}</h1><p>${safeMessage}</p><p>You may close this window and return to the dashboard.</p></body></html>`,
...
export async function GET(request: Request) {
  ...
  try {
    ...
    session = await findOAuthSessionByState(digest, "whoop");
    ...
    if (url.searchParams.has("error")) {
      ...
      return page("Authorization cancelled", "WHOOP access was not granted.", 400);
    }
    ...
    const providerSubjectHash = await hashProviderSubject({
      provider: "whoop",
      providerSubject: profile.userId,
      keyMaterial: requiredConfig("PROVIDER_SUBJECT_HASH_KEY_V1"),
    });
    await completeProviderAuthorization({
    ...
    return page("WHOOP connected", "Your WHOOP account is now connected.");
  } catch {
    ...
    return page("Authorization failed", "WHOOP could not be connected. Please try again.", 500);
  }
}
