...
- **Dense data widens the chart instead of crushing it.** A fixed-width chart with a year of daily points would pack them well under a pixel apart, making the crosshair feel laggy and most points impossible to land on. Every point is kept at least 4px apart by growing the chart's actual width instead; once it's wider than its card, the card scrolls horizontally (drag or trackpad-scroll to see the rest) rather than shrinking points back down. Charts with few points (the common case — Day view, or any Week/Month/Year bucket) are unaffected and stay at the original fixed width. Pointer tracking is also throttled to one update per animation frame so rapid mouse movement doesn't cause jank.
