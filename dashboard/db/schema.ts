...
export const dailySourceRecords = sqliteTable(
  "daily_source_records",
  {
    ...
    bodyTemperatureDeviationC: real("body_temperature_deviation_c"),
  ...
  (table) => [
    ...
    check("ck_daily_source_records_total_calories", sql`${table.totalCalories} IS NULL OR ${table.totalCalories} >= 0`),
    check("ck_daily_source_records_respiratory_rate", sql`${table.respiratoryRate} IS NULL OR ${table.respiratoryRate} > 0`),
    check("ck_daily_source_records_skin_temperature", sql`${table.skinTemperatureC} IS NULL OR ${table.skinTemperatureC} > 0`),
