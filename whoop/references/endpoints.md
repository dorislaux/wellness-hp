...
- **All requests go through a real `curl` subprocess, not Python's `urllib`.** Cloudflare, sitting in front of `api.prod.whoop.com`, rejects Python's own TLS client fingerprint at the edge before WHOOP's application code ever sees the request — observed directly as an HTTP 403 with body `error code: 1010` (Cloudflare's "client/browser signature banned" error), while an identical request via `curl` succeeds. This is a TLS-handshake-level block, unrelated to HTTP headers like `User-Agent`, so `curl_transport` shells out to the system's `curl` binary via `subprocess` (both are already project dependencies — `curl` ships with macOS/Linux by default) rather than trying to fix it in pure Python. If WHOOP's Cloudflare configuration changes and this is no longer needed, that's a simplification to make later, not now.
